...
yelp_tibble <- tibble(____ = yelp_data) # "____" will be the column name
yelp_tibble
...
yelp_df <- yelp_tibble %>% 
  hoist(____, # add name of parent column in original tibble
        "____", # add desired sub-lists
        "____",
...
yelp_df_export <- yelp_df2 %>%
  select("___", # add desired columns
         ) %>%
  head(5)
