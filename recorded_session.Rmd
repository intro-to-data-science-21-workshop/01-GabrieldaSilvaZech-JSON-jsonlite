...
## The art and craft of *rectangling*
...
## Using `hoist()` to select desired columns

The function `hoist()` is very handy as it allows us to pluck out only specific elements within (deeply) nested lists.

```{r}
# convert imported JSON data to a tibble with "business_info" as column name
...
yelp_df <- yelp_df %>% 
  hoist(business_info, # unused sub-lists remain as last column in data frame
        "name",
        "city",
        "neighborhood" = list("neighborhoods", 1), # selects first element in "neighborhoods" list, similar to [["neighborhoods"]][[1]]
        "hours")
...
## Using `unnest_longer()` to unnest lists into new rows
...
First, we want to have a column named `day`` where each row will be a day of the week. This means the table will grow **longer** with each row being a day of the week.
...
## Using `unnest_wider()` to unnest lists into new columns

Now that we have new rows with each day of the week, we want to put the information on the closing and opening time on a column each. This will make the table grow **wider** with two new columns, but no new rows will be created.
...
Exporting data to a JSON file is very simple. First we have to transform the data (e.g. the data frame) into a JSON string with `toJSON()`. Then we write it to file with `write()`.
...
Make them nest something https://tidyr.tidyverse.org/reference/nest.html
